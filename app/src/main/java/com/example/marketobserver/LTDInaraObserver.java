...
public class LTDInaraObserver implements IObserver
{
    ...
    public void init(Intent intent, ObservingService observingService)
    {
        ...
        maximalDistance = Float.parseFloat(intent.getStringExtra(FieldPassNames.MAXIMAL_DISTANCE_PASS_NAME));
        minimalDemand = Integer.parseInt(intent.getStringExtra(FieldPassNames.MINIMAL_DEMAND_PASS_NAME));
        allowMPads = intent.getBooleanExtra(intent.getStringExtra(FieldPassNames.ALLOW_MSIZED_PADS_PASS_NAME), false);
    ...
    private void update(WebsiteConnectionManager connectionManager) throws IOException
    {
        ...
        results.sort((a, b) -> Integer.compare(b.price, a.price));
    ...
    private String recordsToExpandedText(ArrayList<ResultRecord> results)
    {
        ...
        {
            expandedText.append(results.get(i).toString()).append(i < 4 && i < results.size() - 1 ? '\n' : "");
        }
        return expandedText.toString().equals("") ? "No matching results" : expandedText.toString();
    }
