...
public class ObservingService extends Service
{
    ...
    public int onStartCommand(Intent intent, int flags, int startId)
    {
        ...
        },() -> resume(), () -> observer.stopUpdateThread());
    ...
    private void createNotificationChannel()
    {
        ...
        {
            NotificationChannel channel = new NotificationChannel(channelID, "Observing Channel", NotificationManager.IMPORTANCE_DEFAULT);
    ...
    private Intent getDefaultNotificationIntent()
    {
        ...
        intent.putExtra(FieldPassNames.REFERENCE_SYSTEM_PASS_NAME, mainIntent.getStringExtra(FieldPassNames.REFERENCE_SYSTEM_PASS_NAME));
        intent.putExtra(FieldPassNames.MAXIMAL_DISTANCE_PASS_NAME, mainIntent.getStringExtra(FieldPassNames.MAXIMAL_DISTANCE_PASS_NAME));
        intent.putExtra(FieldPassNames.MINIMAL_DEMAND_PASS_NAME, mainIntent.getStringExtra(FieldPassNames.MINIMAL_DEMAND_PASS_NAME));
        intent.putExtra(FieldPassNames.ALLOW_MSIZED_PADS_PASS_NAME, mainIntent.getBooleanExtra(FieldPassNames.ALLOW_MSIZED_PADS_PASS_NAME, false));
        return intent;
    }
    private PendingIntent getDefaultNotificationPendingIntent()
    {
        return PendingIntent.getActivity(this, 0, getDefaultNotificationIntent(), PendingIntent.FLAG_UPDATE_CURRENT);
    }
    ...
    {
        getSystemNotificationService().notify(1, currentNotificationBuilder.setStyle(new NotificationCompat.BigTextStyle().bigText(bigText)).setContentText(content).build());
    }
    ...
    {
        PendingIntent pauseIntent = PendingIntent.getBroadcast(this, 0, new Intent().setAction(FieldPassNames.PAUSE_CODE), PendingIntent.FLAG_UPDATE_CURRENT);
        currentNotificationBuilder.addAction(R.drawable.ic_pause_black_24dp, "Pause", pauseIntent);
    }
    private void addStopButton()
    {
        PendingIntent stopIntent = PendingIntent.getBroadcast(this, 2, new Intent().setAction(FieldPassNames.STOP_CODE), PendingIntent.FLAG_UPDATE_CURRENT);
        currentNotificationBuilder.addAction(R.drawable.ic_stop_observing_service, "Stop", stopIntent);
    }
    private void addResumeButton()
    {
        PendingIntent resumeIntent = PendingIntent.getBroadcast(this, 1, new Intent().setAction(FieldPassNames.RESUME_CODE), PendingIntent.FLAG_UPDATE_CURRENT);
